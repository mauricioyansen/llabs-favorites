...
describe("Favorite a  product (E2E)", () => {
  ...
  test("[POST] /favorite-products", async () => {
    ...
    const { id } = mockProductData[0];

    const res = await request(app.getHttpServer())
      .post(`/favorite-products/${id}/favorite`)
      .set("Authorization", `Bearer ${accessToken}`);

    expect(res.statusCode).toBe(201);

    const favProductOnDb = await prisma.favoriteProduct.findFirst({
      where: { productId: id.toString() },
    });

    expect(favProductOnDb).not.toBeNull();
  });
});
